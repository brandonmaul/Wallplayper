...
public class Model implements Initializable {
    ...
    /** These are javaFX specific variables, in order to modify an xml element the xml needs to have
        an fx:id which we will reference in code here(Model.java). In order to manipulate a desired xml element you need
        to make a variable here and name it the **same name as you put in fx:id** make sure to add an '@FXML'  before the vvariable name,
        this lets the model  java class to link that variable to a specific xml element **/
    @FXML
    ...
    * */
    ...
    /*
    * Will be called once on an implementing model when the contents of its associated document have
